...
public class OffermanagementImpl extends AbstractComponentFacade implements Offermanagement {
  ...
  @Override
  public PaginatedListTo<OfferEto> findBestOfferEtosAtTheMoment(OfferSearchCriteriaTo criteria, LocalDateTime moment) {

    PaginatedListTo<OfferEto> offers = findOfferEtos(criteria);
    SpecialSearchCriteriaTo specialCriteria = new SpecialSearchCriteriaTo();
    specialCriteria.setDateOfCheckingOffers(moment);
    Map<Long, SpecialCto> offerIdToSpecial = new HashMap();
    for (SpecialCto special : findSpecialCtos(specialCriteria)) {
      offerIdToSpecial.put(special.getOffer().getId(), special);
    }
    for (OfferEto offer : offers.getResult()) {
      SpecialCto special = offerIdToSpecial.getOrDefault(offer.getId(), null);
      if (special != null && special.getSpecial().getSpecialPrice().compareTo(offer.getPrice()) < 0) {
        offer.setPrice(special.getSpecial().getSpecialPrice());
      }
    }
    return offers;
  }
  ...
  public SpecialEto saveSpecial(SpecialEto special) {
    ...
    return getBeanMapper().map(this.specialDao.save(getBeanMapper().map(special, SpecialEntity.class)),
        SpecialEto.class);
  }

  @Override
  public List<SpecialEto> findSpecials(SpecialSearchCriteriaTo criteria) {

    return getBeanMapper().mapList(this.specialDao.findActiveSpecials(criteria), SpecialEto.class);
  }

  @Override
  public List<SpecialCto> findSpecialCtos(SpecialSearchCriteriaTo criteria) {
