...
public class OffermanagementImpl extends AbstractComponentFacade implements Offermanagement {
  ...
  public SpecialEto findSpecial(long id) {
    ...
    return getBeanMapper().map(this.specialDao.find(id), SpecialEto.class);
  }

  @Override
  public List<SpecialEto> findSpecialEtos(SpecialSearchCriteriaTo searchCriteriaTo) {

    searchCriteriaTo.limitMaximumPageSize(MAXIMUM_HIT_LIMIT);
    return getBeanMapper().mapList(this.specialDao.findActiveSpecials(searchCriteriaTo), SpecialEto.class);
  }

  @Override
  public SpecialCto findSpecialCto(long id) {

    return getBeanMapper().map(this.specialDao.find(id), SpecialCto.class);
  }

  @Override
  public List<SpecialCto> findSpecialCtos(SpecialSearchCriteriaTo searchCriteriaTo) {

    searchCriteriaTo.limitMaximumPageSize(MAXIMUM_HIT_LIMIT);
    return getBeanMapper().mapList(this.specialDao.findActiveSpecials(searchCriteriaTo), SpecialCto.class);
  }
